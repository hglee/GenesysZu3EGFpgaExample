...
Add constraints in `constratins/Genesys-ZU-3EG-D-Master.xdc`. Make sure I2C mux pin match to name in external pin. In this case, it will be `GPIO_0_0_tri`. GPIO LED will be treat as internally.
...
Specify exported xsa directory.
...
Edit `project-spec/meta-user/meta-xilinx-tools/recipes-bsp/uboot-device-tree/files/system-user.dtsi` before build. Below file was taken from https://github.com/Digilent/Genesys-ZU and edited for 2023.2.
...
Make sure there is no boot image on SD card.
...
For example, if you server is 192.168.0.18,
...
Check `/sys/class/gpio` to find target device.

For example, you need to use `gpiochip504` because address of GPIO LED is 0x80000000. You can check this address in vivado.
...
Export and change direction of 4 GPIO with root permission(`sudo su`).
...
You can control LED by setting value with root permission.
